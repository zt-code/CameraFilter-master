...
public class CameraController implements ICamera{
    ...
    public void open(int cameraId) {
        try {
            ...
            if (mCamera != null){
                ...
                if(cameraId == 0) { //后置摄像头
                    param.setFocusMode(Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO); //自动聚焦
                }else {
                    List<String> focusModes = param.getSupportedFocusModes();
                    for (String mode : focusModes) {  //检查有没有自动对焦，有些手机没有（比如小米某些型号）
                        if (mode.equals(Camera.Parameters.FOCUS_MODE_AUTO)) {
                            param.setFocusMode(Camera.Parameters.FOCUS_MODE_AUTO); //自动聚焦
                            break;
                        }
                    }
                }
            ...
            //T.showLong(ActivityManager.getInstance().getLastActivity(), "请在设置中开启相机权限");
        }
    }
    ...
    public void onFocus(Point point, Camera.AutoFocusCallback callback) {
        ...
        //再次进行转换
        point.x= (int) (((float)point.x)/ OpenGL.screenWidth*2000-1000);
        point.y= (int) (((float)point.y)/OpenGL.screenHeight*2000-1000);
