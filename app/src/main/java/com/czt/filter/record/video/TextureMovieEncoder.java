...
package com.czt.filter.record.video;
...
import com.czt.filter.gpufilter.basefilter.MagicCameraInputFilter;
import com.czt.filter.lisenter.RecordStopLisenter;
import com.czt.filter.record.gles.EglCore;
...
public class TextureMovieEncoder implements Runnable {
    ...
    private RecordStopLisenter recordStopLisenter; //录制完成监听回调
    ...
    /**
     *告诉录像机开始录音。（从非编码器线程调用）。
     *创建一个新线程，该线程将使用所提供的配置创建编码器。
     *在记录器线程已启动并准备接受消息后返回。这个
     *编码器可能尚未完全配置。
     */
    public void startRecording(EncoderConfig config) {
        ...
        synchronized (mReadyFence) {
            if (mRunning) {
                Log.i(TAG, "Encoder thread already running");
                return;
            }
            mRunning = true;
    ...
    public void pauseRecording(){
        try{
            ...
            e.printStackTrace();
            Log.i("why", "pauseRecording:"+e.toString());
        }
    }
    public void resumeRecording(){
        try{
            ...
            e.printStackTrace();
            Log.i("why", "resumeRecording:"+e.toString());
        }
    }
    /**
    ...
    public void frameAvailable(SurfaceTexture st) {
        ...
        if (timestamp == 0) {
            ...
            Log.i(TAG, "HEY: got SurfaceTexture with timestamp of zero");
            return;
        }
    ...
    private static class EncoderHandler extends Handler {
        ...
        public void handleMessage(Message inputMessage) {
            ...
            if (encoder == null) {
                Log.i(TAG, "EncoderHandler.handleMessage: encoder is null");
                return;
            }
    ...
    private void handleFrameAvailable(float[] transform, long timestampNanos) {
        Log.i(TAG, "handleFrameAvailable tr=" + transform);
...
//            type = MagicFilterType.NONE;
        ...
        if(recordStopLisenter != null) recordStopLisenter.onCompletion();
    }
//    private MagicFilterType type = MagicFilterType.NONE;
    private AFilter mShowFilter = new NoFilter(OpenGL.context.getResources());
